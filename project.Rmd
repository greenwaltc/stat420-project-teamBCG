...
```{r}
# TODO: backwards BIC for crop year factor have a long runtime - consider removing them?
# TODO: clean up code
# TODO: Remove TODO's
```
...
We also want to evaluate whether our models can be made simpler, so we perform AIC and BIC on them.  

```{r}
multi.aic.backward.bc.cropyear_cont <- step(multi.full.bc.cropyear_cont, direction = "backward", trace = 0)
output_diagnostics(multi.aic.backward.bc.cropyear_cont, vif = 0)
```
...
```{r}
multi.aic.forward.bc.cropyear_cont <- step(lm(bc_transform(Production, multi.full.bc.cropyear_cont_lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
...
output_diagnostics(multi.aic.forward.bc.cropyear_cont, vif = 0)
```
...
multi.full.bc.cropyear_cat = lm(paste("bc_transform(Production, multi.full.cropyear_cat_lambda) ~ ",
...
```{r}
multi.aic.forward.bc.cropyear_cat <- step(lm(bc_transform(Production, multi.full.cropyear_cat_lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production, multi.full.cropyear_cat_lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cat),
                                        which = "term.labels"),
                                   collapse = " + "))
                                           ),
...
```{r}
multi.bic.forward.bc.cropyear_cat <- step(lm(bc_transform(Production, multi.full.cropyear_cat_lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production, multi.full.cropyear_cat_lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cat),
                                        which = "term.labels"),
...
models.cropyear_cont = list(
  multi.full.cropyear_cont, multi.full.bc.cropyear_cont, multi.aic.backward.bc.cropyear_cont,
               multi.bic.backward.bc.cropyear_cont, multi.aic.forward.bc.cropyear_cont,
               multi.bic.forward.bc.cropyear_cont
  )

models.cropyear_cat = list(
  multi.full.cropyear_cat, multi.full.bc.cropyear_cat, multi.aic.backward.bc.cropyear_cat,
               multi.bic.backward.bc.cropyear_cat, multi.aic.forward.bc.cropyear_cat,
               multi.bic.forward.bc.cropyear_cat
  )


models.cropyear_omit = list(
  multi.full.cropyear_omit, multi.full.bc.cropyear_omit, multi.aic.backward.bc.cropyear_omit,
               multi.bic.backward.bc.cropyear_omit, multi.aic.forward.bc.cropyear_omit,
               multi.bic.forward.bc.cropyear_omit
  )

models.cropyear_cont.lambdas = rep(multi.full.bc.cropyear_cont_lambda, 6)
models.cropyear_cont.lambdas[1] = -1

models.cropyear_cat.lambdas = rep(multi.full.bc.cropyear_cat_lambda, 6)
models.cropyear_cat.lambdas[1] = -1

models.cropyear_omit.lambdas = rep(multi.full.bc.cropyear_omit_lambda, 6)
models.cropyear_omit.lambdas[1] = -1
...
for (i in 1:length(models.cropyear_cont)) {
  ...
  raw_preds = predict(models.cropyear_cont[[i]], newdata = crops.test.cropyear_cont)
  if(lambda == -1){
    predictions = raw_preds
  } else {
    predictions = inverse_bc_transform(
      raw_preds,
      models.cropyear_cat.lambdas[i]
    )
  }

  test_rmses.cropyear_cont[i] = sqrt(
    mean(
      (crops.test.cropyear_cont$Production - predictions)^2
    )
  )
...
for (i in 1:length(models.cropyear_cat)) {
  ...
  raw_preds = predict(models.cropyear_cat[[i]], newdata = crops.test.cropyear_cat)
  if(lambda == -1){
    predictions = raw_preds
  } else {
    predictions = inverse_bc_transform(
  ...
  test_rmses.cropyear_cat[i] = sqrt(
    mean(
      (crops.test.cropyear_cat$Production - predictions)^2
    )
  )
...
for (i in 1:length(models.cropyear_omit)) {
  ...
  raw_preds = predict(models.cropyear_omit[[i]], newdata = crops.test.cropyear_omit)
  if(lambda == -1){
    predictions = raw_preds
  } else {
    predictions = inverse_bc_transform(
      raw_preds,
      models.cropyear_omit.lambdas[i]
    )
  }
  
  test_rmses.cropyear_omit[i] = sqrt(
    mean(
      (crops.test.cropyear_omit$Production - predictions)^2
    )
  )
