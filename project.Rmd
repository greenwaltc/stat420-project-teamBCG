...
```{r}
crops.train.poly.no_infl <- crops.train[!idx_infl_pts(baseline.poly),]
...
***
```{r}
multi.form = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1) +
                        ...
                        I(log(Pesticide))) *
                        (Crop + Season + Zone + Crop_Year))
multi.full = lm(multi.form, data = crops.train)
output_diagnostics(multi.full, vif = FALSE)
```
...
crops.train.multi.no_infl <- crops.train[!idx_infl_pts(multi.full),]

multi.full = lm(multi.form, data = crops.train.multi.no_infl)
output_diagnostics(multi.full, vif = FALSE)
```

```{r}
bc <- boxcox(multi.full)
(lambda <- bc$x[which.max(bc$y)])

multi.full.bc = lm(paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form),
                                        which = "term.labels"),
                                   collapse = " + ")),
                        data = crops.train.multi.no_infl)
output_diagnostics(multi.full.bc, vif = FALSE)
```

***

```{r}
# multi.aic.backward.bc <- step(multi.full.bc, direction = "backward", trace = 0)
# output_diagnostics(multi.aic.backward.bc, vif = 0)
```

```{r}
# multi.bic.backward.bc <- step(multi.full.bc,
#   direction = "backward", trace = 0,
#   k = log(nrow(crops.train.multi.no_infl))
# )
# output_diagnostics(multi.bic.backward.bc, vif = 0)
```

```{r}
multi.aic.forward.bc <- step(multi.full.bc, direction = "forward", trace = 0)
output_diagnostics(multi.aic.forward.bc, vif = 0)
```

```{r}
multi.bic.forward.bc <- step(multi.full.bc,
  direction = "forward", trace = 0,
  k = log(nrow(crops.train.multi.no_infl))
)
output_diagnostics(multi.bic.forward.bc, vif = 0)
```
...
```{r}
models = list(multi.full, multi.full.bc, multi.aic.backward.bc, 
               multi.bic.backward.bc, multi.aic.forward.bc,
               multi.bic.forward.bc)
modelNames = c("multi.full", "multi.full.bc", "multi.aic.backward.bc", 
               "multi.bic.backward.bc", "multi.aic.forward.bc",
               "multi.bic.forward.bc")
```
...
train_rmses = rep(0,length(models))
test_rmses = rep(0,length(models))
train_adjr2 = rep(0,length(models))
for (i in 1:length(models)) {
  loocv_rmses[i] = get_loocv_rmse(models[[i]])
  train_rmses[i] = sqrt(mean(resid(models[[i]])^2))
  test_rmses[i] = sqrt(
    mean(
      (crops.test$Production - predict(models[[i]], newdata = crops.test))^2
    )
  )
  train_adjr2[i] = summary(models[[i]])$adj.r.squared
}

kable(data.frame("Model" = modelNames, "LOOCV-RMSE" = loocv_rmses,
                 "Train RMSE" = train_rmses, "Test RMSE" = test_rmses,
                 "Train Adj R-Squared" = train_adjr2))
```
