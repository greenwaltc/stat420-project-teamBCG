...
output_diagnostics(baseline.poly)
nrow(crops)
...
baseline.poly.bc <- lm(
  bc_transform(Production + 1, lambda) ~
...
output_diagnostics(baseline.poly.bc)
```
...
output_diagnostics(model.aic.backward)
```

```{r}
model.bic.backward <- step(baseline.poly.bc,
  direction = "backward", trace = 0,
  k = log(nrow(crops.train.poly.no_infl))
)
output_diagnostics(model.bic.backward)
```
...
Here we summarize LOOCV-RMSE for each of our models:
