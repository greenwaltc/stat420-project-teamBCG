...
# TODO: inverse boxcox
# TODO: clean up code
# TODO: In results table, include a column with # of features
# TODO: Actually write the document
```
...
inverse_transform_for_predictions = function(x, lambda) {
  # (x*λ+1)^(1/λ)
}
```
...
baseline.cropyear_cont <- lm(I(Production + 1) ~ ., data = crops.train.no_infl)

bc <- boxcox(baseline.cropyear_cont)
(lambda <- bc$x[which.max(bc$y)])

baseline.bc.cropyear_cont <- lm(bc_transform(Production + 1, lambda) ~ ., data = crops.train.no_infl)
...
baseline.poly.form.cropyear_cont = as.formula(Production + 1 ~ 
...
baseline.poly.bc.cropyear_cont = lm(paste("bc_transform(Production + 1, lambda) ~ ", 
...
***
```{r}
multi.form.cropyear_cont = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1) +
...
multi.full.bc.cropyear_cont = lm(paste("bc_transform(Production + 1, lambda) ~ ",
...
```{r}
multi.aic.forward.bc.cropyear_cont <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cont),
                                        which = "term.labels"),
                                   collapse = " + "))
                                           ),
...
```{r}
multi.bic.forward.bc.cropyear_cont <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cont),
                                        which = "term.labels"),
...
# Box-cox requires that the response be positive (0 doesn't work), so we add
# 1 here (just need to remember to subtract 1 when giving actual predictions)
baseline.cropyear_cat <- lm(I(Production + 1) ~ ., data = crops.train.no_infl)

bc <- boxcox(baseline.cropyear_cat)
(lambda <- bc$x[which.max(bc$y)])

baseline.bc.cropyear_cat <- lm(bc_transform(Production + 1, lambda) ~ ., data = crops.train.no_infl)
...
```{r}
# continuous-categorical interactions
baseline.poly.form.cropyear_cat = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1)) *
...
baseline.poly.bc.cropyear_cat = lm(paste("bc_transform(Production + 1, lambda) ~ ", 
...
***
```{r}
multi.form.cropyear_cat = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1) +
...
multi.full.bc.cropyear_cat = lm(paste("bc_transform(Production + 1, lambda) ~ ",
...
```{r}
multi.aic.forward.bc.cropyear_cat <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cat),
                                        which = "term.labels"),
...
```{r}
multi.bic.forward.bc.cropyear_cat <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_cat),
                                        which = "term.labels"),
...
# Box-cox requires that the response be positive (0 doesn't work), so we add
# 1 here (just need to remember to subtract 1 when giving actual predictions)
baseline.cropyear_omit <- lm(I(Production + 1) ~ ., data = crops.train.no_infl)

bc <- boxcox(baseline.cropyear_omit)
(lambda <- bc$x[which.max(bc$y)])

baseline.bc.cropyear_omit <- lm(bc_transform(Production + 1, lambda) ~ ., data = crops.train.no_infl)
...
```{r}
# continuous-categorical interactions
baseline.poly.form.cropyear_omit = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1)) *
...
baseline.poly.bc.cropyear_omit = lm(paste("bc_transform(Production + 1, lambda) ~ ", 
...
***
```{r}
multi.form.cropyear_omit = as.formula(Production + 1 ~ 
                        (poly(Area, degree = 2, raw = 1) +
                        poly(Annual_Rainfall, degree = 2, raw = 1) +
                        poly(Fertilizer, degree = 2, raw = 1) +
                        poly(Pesticide, degree = 2, raw = 1) +
...
multi.full.bc.cropyear_omit = lm(paste("bc_transform(Production + 1, lambda) ~ ",
...
```{r}
multi.aic.forward.bc.cropyear_omit <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_omit),
                                        which = "term.labels"),
...
```{r}
multi.bic.forward.bc.cropyear_omit <- step(lm(bc_transform(Production + 1, lambda) ~ 1, data = crops.train.multi.no_infl),
                                           scope = as.formula(
                                             paste("bc_transform(Production + 1, lambda) ~ ",
                             paste(attr(terms(multi.form.cropyear_omit),
                                        which = "term.labels"),
...
for (i in 1:length(models.cropyear_cont)) {
  ...
  test_rmses.cropyear_cont[i] = sqrt(
    mean(
      (crops.test.cropyear_cont$Production - predict(models.cropyear_cont[[i]], newdata = crops.test.cropyear_cont))^2
    )
  )
...
for (i in 1:length(models.cropyear_cat)) {
  ...
  test_rmses.cropyear_cat[i] = sqrt(
    mean(
      (crops.test.cropyear_cat$Production - predict(models.cropyear_cat[[i]], newdata = crops.test.cropyear_cat))^2
    )
  )
...
for (i in 1:length(models.cropyear_omit)) {
  ...
  test_rmses.cropyear_omit[i] = sqrt(
    mean(
      (crops.test.cropyear_omit$Production - predict(models.cropyear_omit[[i]], newdata = crops.test.cropyear_omit))^2
    )
  )
