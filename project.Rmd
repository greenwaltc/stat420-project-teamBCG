...
After examining LINE assumptions and predictive performance against the test data we would select the "multi.bic.forward.bc.cropyear_cont" model, with "multi.bic.forward.bc.cropyear_cat" being a very close second.  These models also had the lowest number of predictors, with the other models over fitting relative to these.  

It turns out that whether the crop year variable is categorical or continuous did not differentiate the models on its own, but it is very important for predictive power.  When omitted the model(s) performed significantly worse.  
